...
# Depending on the platform on which you want to run the brm you might need this or not. 
# We ran the models on a Linux-operated server
options(mc.cores = 4,
...
```{r}
# Load dataset after basic cleaning and labelling 
# (see github.com/synth-science/synth-rep-dataset for the raw data and basic cleaning code)
...
degfield_hierarchy <- readxl::read_excel("data/raw/sb/degfield_hierarchy.xlsx") %>% 
  # Contains a classification of the general codes into larger categories based on 
  # this wikipedia article: https://en.wikipedia.org/wiki/Outline_of_academic_disciplines
  mutate(across(degfield_label:degfield_alternative_category, str_to_lower))
...
The original `usa_00015.dta.gz` file for the census data was too large to upload to GitHub, which has a file-size limit. The file is available for download from our OSF project at https://osf.io/2r9sy. The code below will download the `usa_00015.dta.gz` and save it in the `data/raw/us_census` file directory. This code presumes you have downloaded the entire RStudio project from our GitHub repo at https://github.com/TaymAlsalti/MRP_Norming_Tutorial, which already contains the `data/raw/us_census` file directory structure. As an alternative, you could alter or omit the `path` argument within the `osf_download()` function, and save the `usa_00015.dta.gz` file elsewhere on your local computer.

```{r, eval = FALSE}
# Download the 143.3 MB file, and save in a subfolder
osf_retrieve_node("https://osf.io/2r9sy") |>
  osf_ls_files(path = "Large data files/us_census/raw") |> 
  osf_download(path = "data/raw/us_census")
```

Now load the census data file into R, process, and save as `census_sb`.
...
brm_1 <- brm(
  bf(
    wordsum ~ s(age, by = educ) + (1 | educ) + female + (1 | race) + hispan + (1 | region_residence) + (1 | marst) + (1 | degfield_branch) + (1 | occ_category) + (1 | region_birth) + (1 | income_brackets) +
      (1 | educ:race) +
      (1 | educ:hispan) +
...
brm_2 <- brm(
  ...
  prior = prior(normal(0, 1), class = "b") +   # Coefficients of fixed effects
    ...
    prior(exponential(1), class = "sds"),      # Wiggliness parameters
  seed = 14,
  chains = 4,  # Default 
  iter = 2000, # Default
...
m_4 <- brm(
  bf(
    wordsum_ordinal ~ s(age) + (1 | educ) + (1 | race) + hispan + (1 | region_residence) + (1 | marst) + (1 | degfield_branch) + (1 | occ_category) + (1 | region_birth) + female + (1 | income_brackets),
...
```{r}
brm_5_v2 <- brm(
  bf(wordsum ~ s(age) + (1 | educ) + (1 | race) + hispan + (1 | region_residence) + (1 | marst) + (1 | degfield_branch) + (1 | occ_category) + (1 | region_birth) + female + (1 | income_brackets)),
  data = sb_data,
  family = negbinomial(),
  prior = prior(normal(0, 1), class = "b") +
    # prior(normal(0, 3), class = "Intercept") +
...
```{r}
brm_7 <- brm(
  bf(wordsum | trials(10) ~ s(age) + (1 | educ) + (1 | race) + hispan + (1 | region_residence) + (1 | marst) + (1 | degfield_branch) + (1 | occ_category) + (1 | region_birth) + female + (1 | income_brackets)),
  data = sb_data,
  family = binomial(),  
  prior = prior(normal(0, 1), class = "b") +
    # prior(normal(0, 3), class = "Intercept") +
...
```{r}
brm_7_ab <- brm(
  bf(wordsum | trials(10) ~ s(age) + (1 | educ) + (1 | race) + hispan + (1 | region_residence) + (1 | marst) + (1 | degfield_branch) + (1 | occ_category) + (1 | region_birth) + female + (1 | income_brackets) + (1 | pid)),
  data = sb_data,
  family = binomial(),  
  prior = prior(normal(0, 1), class = "b") +
    # prior(normal(0, 3), class = "Intercept") +
...
```{r}
sb_data <- sb_data %>% 
  mutate(wordsum_prop = wordsum / 11) %>% 
  filter(wordsum_prop != 0) # exclude one person with a score of 0

brm_8 <- brm(
  bf(
    wordsum_prop ~ s(age) + (1 | educ) + (1 | race) + hispan + (1 | region_residence) + (1 | marst) + (1 | degfield_branch) + (1 | occ_category) + (1 | region_birth) + female + (1 | income_brackets),
...
# brm_9 <- brm(
#   bf(
#     wordsum | trials(10) ~ s(age) + (1 | educ) + (1 | race) + hispan + (1 | region_residence) + (1 | marst) + (1 | degfield_branch) + (1 | occ_category) + (1 | region_birth) + female + (1 | income_brackets),
#     phi                  ~ s(age) + (1 | educ) + (1 | race) + hispan + (1 | region_residence) + (1 | marst) + (1 | degfield_branch) + (1 | occ_category) + (1 | region_birth) + female + (1 | income_brackets)
#     ),
#   data = sb_data,
...
p <- ppc_pit_ecdf_grouped(t$wordsum, yrep, group = as.factor(t$age_group), prob = 0.99, plot_diff = FALSE)
...
iqs_linear <- sim_pop_sample_with_draws %>% 
  ...
  summarise(RPP_IQ = round(mean(RPP_IQ)),
            RPP_T = round(mean(RPP_T)))
...
prediction_transform = list(
  function(x) round(pmax(0, pmin(10, x))) # For handling normal predictions (that go out of wordsum bounds)
)
...
mean(iqs$RPP_IQ-iqs$Raw_IQ, na.rm = TRUE)

mean(abs(iqs$RPP_IQ-iqs$Raw_IQ), na.rm = TRUE)

max(iqs$RPP_IQ-iqs$Raw_IQ, na.rm = TRUE)
```
...
sb_data_CR_cleaned <- sb_data %>% 
  ...
  mutate(psychsyn = psychsyn(select(., starts_with(main_qs))),
         psychant = psychant(select(., starts_with(main_qs))),
				 ...
				 CR_mahal_outlier = mahad(select(., starts_with(main_qs)), flag = TRUE, confidence = .999, plot = FALSE)$flagged,
...
Negligible differences despite dropping 85/478 data points. 
